...
func NewRootCmd() (*cobra.Command, parallel.EncodingConfig) {
	var (
		config  = parallel.MakeEncodingConfig()
		context = client.Context{}.
			...
			WithHomeDir(parallel.DefaultNodeHome)
	)

	cobra.EnableCommandSorting = false
	root := &cobra.Command{
		Use:   "parallel",
		Short: "Parallel Hotel",
...
func initRootCmd(rootCmd *cobra.Command, encoding parallel.EncodingConfig) {
	rootCmd.AddCommand(
		genutilcli.InitCmd(parallel.ModuleBasics, parallel.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(banktypes.GenesisBalancesIterator{}, parallel.DefaultNodeHome),
		genutilcli.GenTxCmd(parallel.ModuleBasics, encoding.TxConfig, banktypes.GenesisBalancesIterator{}, parallel.DefaultNodeHome),
		genutilcli.ValidateGenesisCmd(parallel.ModuleBasics),
		AddGenesisAccountCmd(parallel.DefaultNodeHome),
		AddGenesisWasmMsgCmd(parallel.DefaultNodeHome),
		tmcli.NewCompletionCmd(rootCmd, true),
		testnetCmd(parallel.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
	)

	server.AddCommands(rootCmd, parallel.DefaultNodeHome, appCreatorFunc, appExportFunc, addModuleInitFlags)
	rootCmd.AddCommand(
		rpc.StatusCommand(),
		queryCommand(),
		txCommand(),
		keys.Commands(parallel.DefaultNodeHome),
	)
}
...
func queryCommand() *cobra.Command {
	...
	parallel.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	parallel.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")
...
func appCreatorFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, options servertypes.AppOptions) servertypes.Application {
	...
	return parallel.New(
		...
		parallel.MakeEncodingConfig(),
		options,
		parallel.GetWasmEnabledProposals(),
		wasmOpts,
...
func appExportFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, height int64,
	...
	var app *parallel.App
	if height != -1 {
		app = parallel.New(logger, db, tracer, false, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, parallel.GetWasmEnabledProposals(), emptyWasmOpts)

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = parallel.New(logger, db, tracer, true, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, parallel.GetWasmEnabledProposals(), emptyWasmOpts)
	}
